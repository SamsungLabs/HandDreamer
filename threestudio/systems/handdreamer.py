...
@threestudio.register("handdreamer-system")
class DreamWaltz(BaseLift3DSystem):
    ...
    class Config(BaseLift3DSystem.Config):
        ...
        stage3_resume: str = ""
        stage3_tuner: bool = False
        stage3_manoGuide: bool = False
    ...
    def forward(self, batch: Dict[str, Any], stage3 = False) -> Dict[str, Any]:
        if stage3 == True:
            #threestudio.info('Refernece nerf rays')
            render_out = self.renderer_ref(**batch)
        else:
            #threestudio.info('Trainable nerf rays')
    ...
    def process_weight_sum_map(self, ws):
        filtered = cv2.bilateralFilter(ws, d=9, sigmaColor=75, sigmaSpace=75)
        #unsharp masking-> edge enhancement
        blurred = cv2.GaussianBlur(filtered, (9, 9), sigmaX=10)
        sharpen_alpha = 1.5
        enhanced_ws = cv2.addWeighted(filtered, 1+sharpen_alpha, blurred, -sharpen_alpha, 0)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.cfg.stage == "warmup" or (
            self.true_global_step < self.cfg.warmup_steps and not self.cfg.skip_warmup and not self.cfg.stage3_tuner
        ):
            # warmup
            assert (
            ...
            assert (
                "depth" in batch
            ), "Depth is required for MANO guidance loss."
            gt_depths = batch["depth"].to(out["opacity"].dtype)
            
            img_mask_mano = (gt_depths.detach() > 1e-6).float()
            img_mask_mano, _ = torch.max(img_mask_mano, dim=3)
            img_mask_mano = img_mask_mano.unsqueeze(0)
            img_mask_mano = torch.nn.functional.interpolate(img_mask_mano, size=(256, 256), mode='nearest')
            img_mask_mano = torch.permute(img_mask_mano, (0,2,3,1))
            if self.cfg.loss.enable_manoloss_guidance:
                loss_l2_mano = F.mse_loss(
                    out["opacity"], img_mask_mano
                )

                cur_mano_wt = self.cfg.loss.lambda_max_mano_guidance - ((self.cfg.loss.lambda_max_mano_guidance - self.cfg.loss.lambda_min_mano_guidance) * np.sqrt(float(self.true_global_step) / float(self.trainer.max_steps)))
                self.log("train/cur_mano_wt", cur_mano_wt)
                self.log("train/loss_l2", loss_l2_mano)
                loss += loss_l2_mano * self.C(cur_mano_wt)
