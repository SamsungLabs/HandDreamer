...
class BaseLift3DSystem(BaseSystem):
    ...
    def configure(self) -> None:
        ...
        print("checkpont: ", self.cfg.stage3_resume)
        if self.cfg.stage3_tuner:
            self.renderer_ref = threestudio.find(self.cfg.renderer_type)(
                self.cfg.renderer,
                geometry=self.geometry,
                material=self.material,
                background=self.background,
            ...
            state_dict, epoch, global_step = load_module_weights(
                    self.cfg.stage3_resume,
                    map_location="cpu",
                )
            print(state_dict)
            self.renderer_ref.load_state_dict(state_dict, strict=False)
            print('\nPrinting laoded params')
            print(self.renderer_ref.state_dict)
            #for name, param in self.renderer_ref.state_dict.items():
            #    print(f"Parameter Name: {name}, Value:\n{param}")
            for param in self.renderer_ref.parameters():
                param.requires_grad = False

            #self.renderer_ref.eval()
            print("Disabling loading ref nerf")
            print("Created Ref Nerf")
