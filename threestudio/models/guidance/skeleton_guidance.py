...
@threestudio.register("skeleton-guidance")
class MultiControlNetGuidance(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        pretrained_model_name_or_path: str = "runwayml/stable-diffusion-v1-5"

        controlnet_preprocessors: List[str] = field(
            default_factory=lambda: ["openpose"]
        )
        controlnet_processors: List[str] = field(
            default_factory=lambda: ["lllyasviel/control_v11p_sd15_openpose"]
        )
        controlnet_cond_scales: List[float] = field(default_factory=lambda: [1.0])
    ...
    def configure(self) -> None:
        threestudio.info(f"Loading Multi-ControlNet...")
        ...
        cond_image_preprocessors = []
        for processor_id in self.cfg.controlnet_preprocessors:
            if processor_id is not None:
                cond_image_preprocessors.append(Processor(processor_id))
            else:
                cond_image_preprocessors.append(None)
        self.cond_image_preprocessors = cond_image_preprocessors

        # Load ControlNet processors
        multi_controlnet_processors = [
            ControlNetModel.from_pretrained(
                controlnet_name_or_path,
                torch_dtype=self.weights_dtype,
            )
            for controlnet_name_or_path in self.cfg.controlnet_processors
        ]
        ...
        pipe_kwargs = {
            ...
            "controlnet": multi_controlnet_processors,
        ...
        threestudio.info(f"Loaded Multi-ControlNet!")
    ...
    def prepare_image_cond(self, cond_rgb: Float[Tensor, "B H W C"], preprocessor):
        if preprocessor is None:
            ...
            for i in range(cond_rgb.shape[0]):
                cond_rgb_ = (
                    (cond_rgb[i].detach().cpu().numpy() * 255).astype(np.uint8).copy()
                )
                detected_map = preprocessor(cond_rgb_)
                control_ = (
                    torch.from_numpy(np.array(detected_map)).float().to(self.device)
                    / 255.0
                )
    ...
    @torch.cuda.amp.autocast(enabled=False)
    def multi_controlnet_forward(
        self,
        sample: Float[Tensor, "..."],
        timestep: Union[int, float, Float[Tensor, "..."]],
        encoder_hidden_states: Float[Tensor, "..."],
        controlnet_cond: List[torch.tensor],
        conditioning_scale: List[float],
    ):
        for i, (image, scale, controlnet) in enumerate(
            zip(controlnet_cond, conditioning_scale, self.controlnet.nets)
        ):
    ...
    def compute_grad_sds(
        ...
        controlnet_cond_images: List[Float[Tensor, "B 3 512 512"]],
        controlnet_cond_scales: List[float],
    ):
        ...
        if prompt_utils.use_perp_neg:
            ...
            with torch.no_grad():
                ...
                ) = self.multi_controlnet_forward(
                    latent_model_input,
                    torch.cat([t] * 4),
                    encoder_hidden_states=text_embeddings,
                    controlnet_cond=controlnet_cond_images,
                    conditioning_scale=controlnet_cond_scales,
                )
    ...
    def __call__(
        ...
        cond_rgbs: List[Float[Tensor, "B H W C"]],
        prompt_utils: PromptProcessorOutput,
        elevation: Float[Tensor, "B"],
        azimuth: Float[Tensor, "B"],
        camera_distances: Float[Tensor, "B"],
        ...
        **kwargs,
        ...
        controlnet_cond_images = [
            self.prepare_image_cond(cond_rgb_, preprocessor)
            for cond_rgb_, preprocessor in zip(cond_rgbs, self.cond_image_preprocessors)
        ]
        controlnet_cond_scales = self.cfg.controlnet_cond_scales
        ...
        if guidance_eval:
            guidance_eval_utils.update(
                {
                    "controlnet_cond_images": controlnet_cond_images,
                    "controlnet_cond_scales": controlnet_cond_scales,
                }
            )
            ...
            guidance_out.update(
                {
                    "controlnet_cond_images": controlnet_cond_images,
                    "controlnet_preprocessors": self.cfg.controlnet_preprocessors,
                }
            )
    ...
    def get_noise_pred(
        ...
        use_perp_neg=False,
        neg_guidance_weights=None,
        controlnet_cond_images=None,
        controlnet_cond_scales=None,
    ):
